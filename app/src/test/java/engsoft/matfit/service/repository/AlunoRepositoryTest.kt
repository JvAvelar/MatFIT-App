...
class AlunoRepositoryTest {
    ...
    private lateinit var alunoRepository: StudentRepository
    ...
    fun getAllStudents() = runTest {
        ...
        whenever(mockRemote.getAllStudents())
            .thenReturn(Response.success(listaAlunos))

        // QUANDO
        val resultado = alunoRepository.getAllStudents()
    ...
    @Test
    fun getAllStudents_listaVazia_retornaListaVazia() = runTest {
    ...
    @Test
    fun getAllStudents_falhaNaAPI_retornaListaVazia() = runTest {
        // DADO -> falha na API
        whenever(mockRemote.getAllStudents())
    ...
    fun cadastrarAluno_sucesso_retornaTrue() = runTest {
        ...
        whenever(mockRemote.registerStudent(aluno))
            .thenReturn(Response.success(alunoEsperado))

        // QUANDO
        val resultado = alunoRepository.registerStudent(aluno)
    ...
    fun deletarAluno_sucesso_retornaTrue() = runTest {
        ...
        whenever(mockRemote.removeStudent(cpf))
            .thenReturn(Response.success(true))

        // QUANDO
        val resultado = alunoRepository.removeStudent(cpf)
    ...
    fun buscarAluno_sucesso_retornaAluno() = runTest {
        ...
        whenever(mockRemote.getStudent(cpf))
            .thenReturn(Response.success(alunoEsperado))

        // QUANDO
        val resultado = alunoRepository.getStudent(cpf)
    ...
    fun atualizarAluno_sucesso_retornaAlunoAtualizado() = runTest {
        ...
        whenever(mockRemote.updateStudent(cpf, dadosAtualizados))
            .thenReturn(Response.success(alunoEsperado))

        // QUANDO
        val resultado = alunoRepository.updateStudent(cpf, dadosAtualizados)
    ...
    fun realizarPagamento_sucesso_retornaTrue() = runTest {
        // Dado -> sucesso
        val cpf = "918.243.680-03"
        whenever(mockRemote.makePayment(cpf))
            .thenReturn(Response.success(true))

        // QUANDO
        val resultado = alunoRepository.makePayment(cpf)
    ...
    fun verificarPagamento_sucesso_retornaAluno() = runTest {
        ...
        whenever(mockRemote.verifyPayment(cpf))
            .thenReturn(Response.success(alunoEsperado))

        // QUANDO
        val resultado = alunoRepository.verifyPayment(cpf)

        // ENTÃO
        assertThat(resultado).isNotNull()
        assertThat(resultado!!.latePayment).isEqualTo(alunoEsperado.latePayment)
        assertThat(resultado).isEqualTo(alunoEsperado)
    }
