...
class UpdateEquipamentoActivity : AppCompatActivity() {
    ...
    private fun atualizar() {
        val id = intent?.getIntExtra(Constants.Equipament.ID, 0) ?: 0
        ...
        if (!baseValidacao.validarNome(nome) || nome.isEmpty())
            ...
            viewModel.atualizarEquipamento(id, Equipament(nome = nome, quantidade = quantidade))
    }


    private fun dadosPadroes() {
        val nome = intent?.getStringExtra(Constants.Equipament.NOME) ?: ""
        val quantidade = intent?.getIntExtra(Constants.Equipament.QUANTIDADE, 0) ?: 0
